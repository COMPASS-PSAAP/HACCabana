#ifndef P3M_FORCE_SOLVER_HPP
#define P3M_FORCE_SOLVER_HPP
...
class P3MForceSolver
{
                       ...
                       const float rsm2 )
  {
    ...
    _cell_list = Cabana::createLinkedCellList( position, 0, aosoa_device.size(),
  ...
  void updateVel( std::shared_ptr<AoSoAType> aosoa_device )
  {
    ...
    Kokkos::parallel_for(
        "copy_bin_index",
        Kokkos::RangePolicy<execution_space>( 0, cell_list.totalBins() ),
        KOKKOS_LAMBDA( const int i )
        {
          int bin_ijk[3];
          cell_list.ijkBinIndex( i, bin_ijk[0], bin_ijk[1], bin_ijk[2] );
          for ( size_t ii = cell_list.binOffset( bin_ijk[0], bin_ijk[1],
                                                 bin_ijk[2] );
    ...
    {
      int bin_ijk[3];
      cell_list.ijkBinIndex( bin_index.access( s, a ), bin_ijk[0], bin_ijk[1],
                             bin_ijk[2] );
      ...
      for ( int ii = -1; ii < 2; ++ii )
      {
        ...
        {
          if ( bin_ijk[1] + jj < 0 ||
               bin_ijk[1] + jj >= cell_list.numBin( 1 ) )
            continue;
          for ( int kk = -1; kk < 2; ++kk )
          {
            if ( bin_ijk[2] + kk < 0 ||
                 bin_ijk[2] + kk >= cell_list.numBin( 2 ) )
              continue;

            const size_t binOffset =
                cell_list.binOffset( bin_ijk[0] + ii, bin_ijk[1] + jj,
                                     bin_ijk[2] + kk );
            const int binSize =
                cell_list.binSize( bin_ijk[0] + ii, bin_ijk[1] + jj,
                                   bin_ijk[2] + kk );
            ...
            for ( int j = binOffset; j < binOffset + binSize; ++j )
            {
              ...
              if ( dist2 < rmax2 )
              {
                ...
                const float tmp =
                    1.0f / Kokkos::sqrt( dist2Err * dist2Err * dist2Err ) -
                    FGridEvalPoly( dist2 );
    ...
    Cabana::SimdPolicy<VECTOR_LENGTH, execution_space> simd_policy(
        0, aosoa_device->size() );
